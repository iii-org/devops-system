...
class ProjectList(Resource):
    ...
    def get(self, user_id):
        if int(user_id) == get_jwt_identity()['user_id'] or get_jwt_identity(
            ...
            output = pjt.get_project_list(logger, app, user_id)
            return output
        else:
...
class ProjectAddMembership(Resource):
    @jwt_required
    def post(self, project_id):
        if get_jwt_identity()["role_id"] in (3, 5):
            parser = reqparse.RequestParser()
            ...
            args = parser.parse_args()
            output = au.project_add_membership(logger, app, project_id, args)
            return output
        else:
...
class ExportToPostman(Resource):
    ...
    def get(self, project_id):
        ...
        output = ci.export_to_postman(logger, app, project_id, jwt_identity)
        return output
...
api.add_resource(ProjectAddMembership, '/project/<int:project_id>/membership')
