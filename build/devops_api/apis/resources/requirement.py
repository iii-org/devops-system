...
class Requirement(object):
    ...
    def get_requirement_by_rqmt_id(self, logger, requirement_id, user_id):
        ...
        get_rqmt_command = db.select([TableRequirement.stru_rqmt.c.flow_info]).where(db.and_(TableRequirement.stru_rqmt.c.id==requirement_id))
    ...
    def del_requirement_by_rqmt_id(self, logger, requirement_id, user_id):
        ...
        update_rqmt_command = db.update(TableRequirement.stru_rqmt).where(db.and_(TableRequirement.stru_rqmt.c.id==requirement_id)).values(disabled=True)
    ...
    # 修改  requirement 內資訊
    def modify_requirement_by_rqmt_id(self, logger, requirement_id,args, user_id):
    

        update_rqmt_command = db.update(TableRequirement.stru_rqmt).where(db.and_(TableRequirement.stru_rqmt.c.id==requirement_id)).values(update_at=datetime.datetime.now(),
            flow_info=self._deal_with_json(args['flow_info'])).returning(TableRequirement.stru_rqmt.c.update_at)
        logger.debug("insert_user_command: {0}".format(update_rqmt_command))
        reMessage = util.callsqlalchemy(self, update_rqmt_command, logger)
    ...
    def get_requirements_by_issue_id(self,logger, issue_id, user_id):
        get_rqmt_command = db.select([TableRequirement.stru_rqmt.c.flow_info]).where(db.and_(TableRequirement.stru_rqmt.c.issue_id==issue_id,TableRequirement.stru_rqmt.c.disabled==False))
        logger.debug("get_rqmt_command: {0}".format(get_rqmt_command))
        result = util.callsqlalchemy(self, get_rqmt_command, logger)
    ...
    def post_requirement_by_issue_id(self, logger,  issue_id, args, user_id):
        ...
        insert_rqmt_command = db.insert(TableRequirement.stru_rqmt).values(project_id=args['project_id'],
            issue_id=issue_id, flow_info=self._deal_with_json(args['flow_info']) ,create_at=datetime.datetime.now(),update_at=datetime.datetime.now() )
