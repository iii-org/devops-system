...
Revision ID: 2ec6730c06de
Revises: 
Create Date: 2020-12-01 10:23:26.038306
...
def upgrade():
    ...
    op.drop_constraint('project_plugin_relation_pk', 'project_plugin_relation', type_='primary')
    op.alter_column('project_plugin_relation', 'project_id',
                    existing_type=sa.INTEGER(),
                    nullable=True)
    op.add_column('project_plugin_relation', sa.Column('id', sa.INTEGER(), primary_key=True))
    ...
    op.alter_column('checkmarx', 'scan_id',
                    existing_type=sa.INTEGER(),
                    nullable=True)
    op.create_foreign_key(None, 'checkmarx', 'project_plugin_relation', ['repo_id'], ['git_repository_id'],
                          ondelete='CASCADE')
    ...
    op.create_foreign_key(None, 'pipeline_software_config', 'pipeline_software', ['software_id'], ['id'],
                          ondelete='CASCADE')
    ...
    op.alter_column('requirements', 'disabled',
                    ...
                    nullable=True,
                    existing_server_default=sa.text('false'))
    op.create_primary_key('requirements_pkey', 'requirements', ['id'])
    ...
    op.alter_column('test_cases', 'disabled',
                    ...
                    nullable=True,
                    existing_server_default=sa.text('false'))
    op.create_primary_key('test_cases_pkey', 'test_cases', ['id'])
    ...
    op.create_primary_key('test_items_pkey', 'test_items', ['id'])
    ...
    op.add_column('test_results', sa.Column('report', sa.String(), nullable=True))
...
def downgrade():
    ...
    op.create_table('group',
                    sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('group_id_seq'::regclass)"),
