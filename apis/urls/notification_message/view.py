...
from resources.apiError import (DevOpsError, argument_error,
                                not_enough_authorization, resource_not_found)
from resources.notification_message import (
    close_notification_message, create_notification_message,
    create_notification_message_reply_slip, delete_notification_message,
    get_notification_message_list, notification_room)
...
class MessageListV2(MethodResource):
    ...
    def get(self):
        ...
        parser.add_argument('offset', type=int, default=0, location="query")
        parser.add_argument('from_date', type=str, location="query")
        parser.add_argument('to_date', type=str, location="query")
        parser.add_argument('search', type=str, location="query")
        parser.add_argument('alert_ids', type=str, location="query")
        parser.add_argument('unread', type=bool, location="query")
        args = parser.parse_args()
        if args["alert_ids"]:
...
class MessageListForAdminV2(MethodResource):
    ...
    def get(self):
        ...
        parser.add_argument('search', type=str, location="query")
        parser.add_argument('alert_ids', type=str, location="query")
        parser.add_argument('include_system_message', type=bool, location="query")
        args = parser.parse_args()
        if args["alert_ids"]:
            args["alert_ids"] = json.loads(args["alert_ids"].replace("\'", "\""))
