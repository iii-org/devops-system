...
class ListMyProjects(Resource):
    ...
    def get(self):
        ...
        parser.add_argument("is_empty_project", type=str, location="args")
...
class SingleProject(Resource):
    ...
    def patch(self, project_id):
        ...
        parser.add_argument("image_auto_del", type=bool, required=False)
        args = parser.parse_args()
        project.check_project_args_patterns(args)
...
class ProjectPluginPod(Resource):
    ...
    def get(self, project_id):
        role.require_in_project(project_id, "Error while getting project info.")
        parser = reqparse.RequestParser()
        parser.add_argument("plugin_name", type=str, location="args")
        args = parser.parse_args()
...
class ProjectUserResourcePodLog(Resource):
    ...
    def get(self, project_id, pod_name):
        role.require_in_project(project_id, "Error while getting project info.")
        parser = reqparse.RequestParser()
        parser.add_argument("container_name", type=str, location="args")
        args = parser.parse_args()
...
class ProjectEnvironmentUrlV2(MethodResource):
    ...
    def get(self, project_id, branch_name):
        role.require_in_project(project_id, "Error while getting project info.")
        return util.success(project.get_kubernetes_namespace_dev_environment_urls(project_id, branch_name))
...
class ProjectUserResourceConfigMapV2(MethodResource):
    ...
    @doc(
        tags=["K8s"],
        description="Delete specific project k8s configmap by configmap_na.",
    )
    ...
    @doc(
        tags=["K8s"],
        description="Update specific project k8s configmap by configmap_name.",
    )
    ...
    @doc(
        tags=["K8s"],
        description="Create specific project k8s configmap by configmap_name.",
    )
    @use_kwargs(router_model.ProjectUserResourceConfigMapsSchema, location="form")
    @marshal_with(router_model.ProjectUserResourceConfigMapResponse)
    @jwt_required()
