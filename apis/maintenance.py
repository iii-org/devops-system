...
def update_db_rancher_projectid_and_pipelineid():
    ...
    rows = db.session.query(ProjectPluginRelation, Project). \
        join(Project).all()
    # print(rows)
    rancher_project_id = ran.rc_get_project_id()
    print("ci_project_id: {0}".format(rancher_project_id))
    for row in rows:
        try:
            ran.rc_disable_project_pipeline( row.ProjectPluginRelation.ci_project_id, \
                row.ProjectPluginRelation.ci_pipeline_id)
            rancher_pipeline_id= ran.rc_enable_project_pipeline(row.Project.http_url)
