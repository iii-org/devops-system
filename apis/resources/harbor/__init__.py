...
def __api_request(method, path, headers=None, params=None, data=None):
    ...
    logger.debug(
        "Harbor api {0} {1}, params={2}, body={5}, response={3} {4}".format(
...
def hb_get_id_by_name(project_name):
    ...
    raise DevOpsError(
        404,
        "Harbor does not have such project.",
...
def hb_create_project(project_name):
    data = {
        ...
        "cve_whitelist": {"items": [{"cve_id": "string"}]},
    ...
    try:
        ...
        if e.unpack_response()["errors"][0]["code"] == "CONFLICT":
            raise DevOpsError(
                422,
                "Harbor already has a project using this name.",
...
def hb_remove_member(project_id, user_id):
    ...
    if member_id is None:
        raise DevOpsError(404, "User is not in the project.", error=apiError.user_not_found(user_id))
...
def hb_list_repo_by_page(project_name: str, page: int = 1, page_size: int = 15) -> list[dict[str:Any]]:
    ...
    for repo in repositories:
        repo["harbor_link"] = hb_build_external_link(
            "/harbor/projects/{0}/repositories/{1}".format(
                repo["project_id"], repo["name"].replace((project_name + "/"), "")
            )
        )
        ret.append(repo)
    return ret
...
def hb_get_artifact(project_name: str, repository_name: str, tag_name: str):
    artifact: dict[str, Any] = __api_get(
        f"/projects/{project_name}/repositories/{__encode(repository_name)}/artifacts/{__encode(tag_name)}",
        params={"with_scan_overview": True},
    ).json()
...
def hb_get_artifact_scan_vulnerabilities_detail(project_name, repository_name, commit_id):
    try:
        artifact = __api_get(
            f"/projects/{project_name}/repositories"
            f"/{__encode(repository_name)}/artifacts/"
            f"{__encode(commit_id)}/additions/vulnerabilities"
        ).json()
...
def hb_copy_artifact_and_re_tag(
    project_name: str,
    from_repo_name: str,
    dest_repo_name: str,
    from_tag: str,
    ...
    try:
        digest: str = hb_get_artifact(project_name, from_repo_name, from_tag)[0]["digest"]
    except Exception:
        logger.info(f"Can not find {from_repo_name}:{from_tag}")
        return

    # if dest_repo:dest_tag is exist, untag it, then re-tag.
    try:
        dest_digest: str = hb_get_artifact(project_name, dest_repo_name, dest_tag)[0]["digest"]
        hb_delete_artifact_tag(project_name, dest_repo_name, dest_digest, dest_tag, keep=True)
        logger.info(f"Replace the old {dest_repo_name}:{dest_digest}")
    except Exception:
        pass

    from_image: str = f"{project_name}/{from_repo_name}@{digest}"
...
def hb_update_repository(project_name, repository_name, args):
    return __api_put(
        f"/projects/{project_name}/repositories/{__encode(repository_name)}",
...
def hb_get_artifact_wtih_digrest(project_name, repository_name, reference):
    try:
        ret = __api_get(
            f"/projects/{project_name}/repositories/{__encode(repository_name)}" f"/artifacts/{reference}"
        ).json()
        return ret
...
def hb_delete_artifact(project_name, repository_name, reference):
    return __api_delete(f"/projects/{project_name}/repositories/{__encode(repository_name)}" f"/artifacts/{reference}")


def hb_list_tags(project_name, repository_name, reference):
    return __api_get(
        f"/projects/{project_name}/repositories/{__encode(repository_name)}" f"/artifacts/{reference}/tags"
    ).json()


def hb_create_artifact_tag(
    project_name: str,
    repository_name: str,
    reference: str,
    tag_name: str,
    ...
    exist_tag_artifact: list = hb_get_artifacts_with_tag(project_name, repository_name, tag_name)
    if exist_tag_artifact:
        if forced:
            hb_delete_artifact_tag(
                project_name,
                repository_name,
                exist_tag_artifact[0]["digest"],
                tag_name,
                keep=True,
            )
        else:
            raise apiError.DevOpsError(
                500,
                f"{tag_name.capitalize()} already exist in this Harbor repository.",
                error=apiError.harbor_tag_already_exist(tag_name, repository_name),
            )

    return __api_post(
        f"/projects/{project_name}/repositories/{__encode(repository_name)}/artifacts/{reference}/tags",
        data={"name": tag_name},
    )


def hb_delete_artifact_tag(
    project_name: str,
    repository_name: str,
    reference: str,
    tag_name: str,
    ...
    tag_num: int = len(hb_list_tags(project_name, repository_name, reference))
    can_remove_artifact: bool = tag_num in [0, 1]
    if tag_num > 0:
        __api_delete(
            f"/projects/{project_name}/repositories/{__encode(repository_name)}/artifacts/{reference}/tags/{tag_name}"
        )
...
def hb_create_registries(args):
    ...
    if args["type"] == "aws-ecr":
        ...
        args["url"] = "{login_server}".format(login_server=args["login_server"])
...
def hb_put_registries(registry_id, args):
    ...
    __api_put(f"/registries/{registry_id}", data=args)
...
def hb_get_replication_policies(args=None):
    if args.get("name", None) is not None:
        ...
        policies = __api_get(f"{replication_polices_base_url}?name={policy_name}").json()
    else:
...
def hb_get_replication_policy(policy_id=None):
    if policy_id:
        policies = __api_get(f"{replication_polices_base_url}/{policy_id}").json()
    else:
        policies = __api_get(replication_polices_base_url).json()
    return policies
...
def hb_put_replication_policy(args, policy_id):
    data = hb_get_replication_policy_data(args)
    __api_put(f"{replication_polices_base_url}/{policy_id}", data=data)
    return policy_id


def hb_delete_replication_policy(policy_id):
    __api_delete(f"{replication_polices_base_url}/{policy_id}")
    return policy_id
...
def hb_execute_replication_policy(policy_id):
    ...
    if dest_registry.get("type") == "aws-ecr":
        account_id = util.AWSEngine(
            dest_credential.get("access_key"), dest_credential.get("access_secret")
        ).get_account_id()
