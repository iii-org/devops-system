...
def get_project_pipeline_version(pj_id):
    ...
    if pj_pipe_version is None:
        row = PipelineUpdateVersion(project_id=pj_id, version=version)
        db.session.add(row)
        db.session.commit()
...
def update_pipeline():
    ...
    if project_repo_names == []:
        project_rows = (
            db.session.query(Project, PipelineUpdateVersion)
            .join(PipelineUpdateVersion, Project.id == PipelineUpdateVersion.project_id)
            .all()
        )
        pj_id_version_mapping = {
            project_row.Project.id: project_row.PipelineUpdateVersion.version for project_row in project_rows
        }
    else:
