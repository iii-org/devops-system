...
class Rancher(object):
    ...
    def rc_get_pipeline_executions(
        self,
        ci_project_id,
        ci_pipeline_id,
        run=None,
        ...
        params = {"order": "desc", "sort": "started", "pipelineId": ci_pipeline_id}
    ...
    def rc_get_pipeline_config(self, ci_pipeline_id, pipelines_exec_run):
        ...
        for index, stage in enumerate(output_execution["pipelineConfig"]["stages"]):
            ...
            for step_index, step in enumerate(stage["steps"]):
                ...
                tmp_step_message.append({"step_id": step_index, "state": step_state})
    ...
    def rc_update_project_pipline(self, project_id, pipeline_id, data):
        self.__api_put(f"/projects/{project_id}/pipelines/{pipeline_id}", data=data)
    ...
    def rc_add_secrets_into_rc_all(self, args):
        ...
        body = {"type": args["type"], "data": data, "labels": {}, "name": args["name"]}
        url = f"/projects/{self.project_id}/secrets"
    ...
    def rc_put_secrets_into_rc_all(self, secret_name, args):
        ...
        for key, value in data.items():
            data[key] = base64.b64encode(bytes(value, encoding="utf-8")).decode("utf-8")
        body = {"type": args["type"], "labels": {}, "name": secret_name, "data": data}
    ...
    def rc_count_each_pj_piplines_by_days(self):
        ...
        for project_plugin_relation in project_plugin_relations:
            ...
            pipline_executions = self.rc_get_pipeline_executions(
                project_plugin_relation.ci_project_id,
                project_plugin_relation.ci_pipeline_id,
            )
            pipline_number = 0
...
def check_pipeline_need_remove(repo_name: str, branch: str, commit_id: str = "") -> None:
    ...
    create_time, del_branch, del_run_num = (
        pipeline_execution.created_at,
        pipeline_execution.del_branch,
        pipeline_execution.run_num,
    )

    pj_id, ci_project_id, ci_pipeline_id = (
        pj_plugin_relation.project_id,
    ...
    if run_num > del_run_num:
        record_log(f"This pipeline has deleted.")
        return
    ...
    record_log(f"Remove {branch}'s pipeline execution, num: {run_num}.")
