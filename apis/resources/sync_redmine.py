...
def round_off_float(num):
    if isinstance(num, float):
        num = str(num)
    return float(
        Decimal(num).quantize(Decimal('0.000'), rounding=ROUND_HALF_UP))
...
def get_complete_percent(project):
    ...
    if project['closed_count'] and project['total_count']:
        complete_percent = round_off_float(project['closed_count'] /
                                           project['total_count'])
...
def get_test_results_count(project_id):
    return model.TestResults.query.filter(
        model.TestResults.run_at < datetime.today(),
        model.TestResults.project_id == project_id).count()
...
def get_project_count_by_user_and_project(user_id, own_project):
    return model.ProjectMember.query.filter(
        ...
        model.ProjectMember.project_id.in_(own_project)).count()
...
def get_project_members_detail(own_project):
    query_collections = get_project_by_current_sync_date(
        detail=True, own_project=own_project)
...
def get_project_members(project_id):
    query_collections = model.ProjectMember.query.filter_by(
...
def get_unclosed_issues_by_user(user_id):
    query_collections = model.RedmineIssue.query.filter_by(
        assigned_to_id=user_id, is_closed=False)
    unclosed_issues = [{
...
def get_postman_passing_rate(detail, own_project):
    ...
    if detail:
        ...
        return sorted(all_passing_rate, key=itemgetter('total'),
                      reverse=True)[:10]
