...
def get_issue(issue_id, with_children=True, journals=True, operator_id=None):
    ...
    if not with_children:
        ...
        for children_issue in issue["children"]:
            ...
            if get_jwt_identity()["role_id"] == role.ADMIN.id or check_user_has_permission_to_see_issue(children_issue["id"]):
...
def common_project_issue_info_serializer(issue: dict[str, Any], with_point: bool = False, download: bool = False):
    ...
    if issue.get("assigned_to") is not None:
        issue["assigned_to"] = get_user_info_by_plan_user_id(issue["assigned_to"]["id"])
    else:
        issue["assigned_to"] = {}

    if issue.get("author") is not None:
        issue["author"] = get_user_info_by_plan_user_id(issue["author"]["id"])
    else:
        issue["author"] = {}
...
def get_user_info_by_plan_user_id(plan_user_id):
    row = (
        ...
        .filter(model.UserPluginRelation.plan_user_id == plan_user_id)
        .first()
    )
    if row is None:
        return {}
...
def get_issue_family(redmine_issue, args={}, all=False, user_name=None, sync=False):
    ...
    default_filters = get_custom_filters_by_args(args=args, children=None)

    output["children"] = get_issue_children(redmine_issue, redmine_obj, default_filters)
