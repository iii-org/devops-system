    ...
    for case in cases:
        ...
        for item in items:
            ...
            try:
                ...
                o_request = {
                    "method": method,
                    "url": {"protocol": scheme, "port": ur_pah.port},
                    "header": [],
                }
            except ValueError:
                return util.respond(
                    400,
                    "url is malformed",
                    {"case_id": case_id, "item_id": item_id, "url": ur_pah},
                )
            ...
            for value in values:
                ...
                if type_id == 1:
                    if location_id == 1:
                        ...
                        o_request_body.append({"key": value["key"], "value": value["value"]})
            ...
            if bool(o_request_body):
                o_request["body"] = {"mode": "formdata", "formdata": o_request_body}
            if bool(o_request):
            ...
            if len(o_execs) > 0:
                o_item["event"] = [
                    {
                        "listen": "test",
                        "script": {"type": "text/javascript", "exec": o_execs},
                    }
                ]
...
def pm_save_result(args):
    ...
    if row is None:
        raise apiError.DevOpsError(404, f"Scan id {scan_id} not found.", apiError.resource_not_found())
