...
class SideexPutSch(Schema):
    ...
    result = fields.Str(
        example={
            ...
            "casesPassed": 1,
            "casesTotal": 1,
            "suites": {"django-sqlite-todo": {"passed": 1, "total": 1}},
        }
    )
...
class SideexPostRes(CommonBasicResponse):
    data = fields.Dict(example={"test_id": 661})
...
class SideexGenerateResultRes(CommonBasicResponse):
    data = fields.List(
        fields.Dict(
            example=[
                {"abc": "x5", "def": "xyz", "xx2": "56"},
                {"abc": "x5", "def": "測試", "xx2": "99"},
            ]
        )
    )
...
class SideexPictStatusRes(CommonBasicResponse):
    data = fields.Dict(example={"finish": True, "branch": "master", "commit_id": "6cdbe6b"})


class SideexCheckResultFileRes(CommonBasicResponse):
    data = fields.Dict(example={"result_file_exist": True})
