...
def checkamrx_keep_report(repo_id, keep_record: int = 5):
    scan = (
        Model.query.with_entities(func.min(Model.run_at).label("run_at"))
        .filter(Model.repo_id == repo_id, Model.report_id > 0)
        .group_by(Model.repo_id)
        .first()
    )
    if scan:
        rows = (
            Model.query.filter(Model.repo_id == repo_id, Model.run_at >= scan.run_at).order_by(desc(Model.run_at)).all()
        )
    else:
