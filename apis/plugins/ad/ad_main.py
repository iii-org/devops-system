...
def generate_base_dn(ldap_parameter, filter_by_ou=True):
    ...
    if ldap_parameter.get("ou") is not None and filter_by_ou is True:
        if isinstance(ldap_parameter.get("ou"), list):
            ...
            search_base += get_search_base_string("ou", ldap_parameter.get("ou").split(","))
    if ldap_parameter.get("domain") is not None:
        search_base += get_search_base_string("dc", ldap_parameter.get("domain").split("."))
...
def create_user(ad_user, login_password):
    ...
    if (
        ad_user.get("iii") is True
        and ad_user.get("userAccountControl") in USER_ACCOUNT_CONTROL_ACTIVE
        and ad_user.get("userPrincipalName") is not None
...
def check_user_by_ad(ad_users, db_users, create_by=None, ldap_parameter=None):
    ...
    for ad_user in ad_users:
        ...
        #  Create user
            ...
            new_user = create_user(ad_user, ldap_parameter.get("default_password"))
...
def check_ad_alive(ldap_parameter=None):
    ...
    try:
        ...
        ret = {"alive": False, "message": error_msg}
    return ret


class AD(object):
    def __init__(self, ldap_parameter, filter_by_ou=False, account=None, password=None):
        self.ad_info = {"is_pass": False, "login": account, "data": {}}
        self.account = None
        self.password = None
        self.server = None
    ...
    def post(self):
        try:
            ...
            if len(res) == 1:
                ...
                res = check_user_by_ad(user, db_user, args.get("ad_type"), ldap_parameter)
            return util.success(res)
        except NoResultFound:
            return util.respond(
                404,
